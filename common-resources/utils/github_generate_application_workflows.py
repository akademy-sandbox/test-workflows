...
def generate_schedule_section(environments):
    """Generates schedule section dynamically (removes empty entries)"""
    schedule_entries = [
        f"    - cron: \"{env['cron_schedule']}\""  # 4-space indentation for YAML formatting
...
def generate_application_workflow(application, config, template):
    """Generates a workflow for a specific application"""
    
    workflow_name = f"{application['name']} - Automation Workflow"
    
    automation_tasks = [task["name"] for task in application["automation_tasks"]]
    default_automation_task = get_default_value(automation_tasks)

    actions = [action for task in application["automation_tasks"] for action in task["actions"]]
    ...
    # Fix: Ensure `application` is correctly set
...
def main():
    print("Generating Application Workflows...")
    ...
    for application in config["applications"]:
        ...
        workflow_filename = os.path.join(args.output, f"workflow_{app_name}.yml")

        print(f"Processing: Application: {app_name}")

        workflow_content = generate_application_workflow(application, config, template)
        ...
        if workflow_content:
            ...
            print(f"Skipped workflow generation for {app_name}")
